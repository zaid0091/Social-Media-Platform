...
export default function FollowersPage() {
  ...
  // 2. Fetch followers list using SWR
  const profileId = profile?.id;
  const { data: followersData, error: followersError, isLoading: loadingFollowers, mutate } = useSWR(
    profileId ? `/users/${profileId}/followers/?page=${page}` : null,
    fetcher
  );

  useEffect(() => {
    if (followersData) {
      const results = followersData.results || [];
      setFollowers((prev) => page === 1 ? results : [...prev, ...results]);
      setHasNext(!!followersData.next);
    }
  }, [followersData, page]);
  ...
  if (profileError || followersError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-50">Followers List Not Available</h2>
        <p className="text-sm text-zinc-500 text-center max-w-xs">
          The requested followers list could not be loaded or you do not have permission to view it.
        </p>
        <button
          onClick={() => router.back()}
        ...
        <header className="flex items-center space-x-4 border-b border-zinc-150 dark:border-zinc-850 pb-4 text-left">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-black text-zinc-900 dark:text-zinc-50 leading-tight">
              {isSelf ? 'My Followers' : `Followers of @${username}`}
            </h1>
            <p className="text-xs text-zinc-400 font-semibold leading-none mt-0.5">
              {profile?.follower_count || 0} followers total
            </p>
          </div>
        </header>

        {/* LIST FEED */}
        {loadingProfile || (loadingFollowers && followers.length === 0) ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <p className="text-sm font-bold text-zinc-450">No followers yet</p>
            <p className="text-xs text-zinc-400 mt-1">When people follow this profile, they will appear here</p>
          </div>
        ) : (
                ...
                >
                  ...
                  {currentUser && u.id !== currentUser.id && (
                    <FollowButton
                      ...
                      onStateChange={() => {
                        // Dynamically update followers view cache
                        mutate();
                      }}
                    />
                  )}
